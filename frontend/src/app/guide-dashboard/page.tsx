...
export default function GuideDashboard() {
  const { apiUrl, token, user, logout } = useApp();
  ...
  // Check role protection - redirect unauthorized users
  useEffect(() => {
    if (!token || !user) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-950 text-white font-sans">
        <div className="text-center space-y-4">
          <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-amber-500 mx-auto" />
          <p className="text-xs text-slate-400">Verifying authorization credentials...</p>
        </div>
      </div>
    );
  }
