...
import { Globe2, MapPin, Calendar, Clock, Star, Landmark, Send, CheckCircle2, XCircle, ShieldCheck, Mail, Compass, Thermometer } from 'lucide-react';
...
export default function InternationalTourDetailPage() {
  const { id } = useParams();
  ...
  // Tabs & Accordions
  const [activeTab, setActiveTab] = useState<'itinerary' | 'visa' | 'reviews'>('itinerary');
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-32 text-center space-y-4 bg-slate-950 text-slate-100 min-h-screen">
        ...
        <p className="text-xs text-slate-400">Loading global itinerary...</p>
      </div>
    );
  }
        ...
        <div className="absolute bottom-6 left-4 right-4 max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div className="space-y-2">
            <span className="rounded bg-secondary px-3 py-1 text-4xs font-bold text-slate-950 uppercase tracking-widest">
              🌍 International Gateway • {pkg.type}
            </span>
            ...
            <p className="text-xs text-slate-300 flex items-center space-x-1">
              ...
              <span>Country Location: {pkg.destination}</span>
            </p>
          </div>
      ...
      <section className="bg-slate-900/40 border-b border-secondary/10 py-5">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          ...
          <div>
            ...
            <span className="text-4xs text-slate-400 uppercase font-bold">Visa Guidance</span>
            <span className="text-xs font-bold block">{pkg.visaRequirement}</span>
          </div>
          <div>
            ...
            <span className="text-xs font-bold block">{pkg.currency || 'USD'}</span>
          </div>
        </div>
      </section>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="flex space-x-6 border-b border-secondary/10 pb-3">
            {[
              ...
              { label: 'Weather & Visa Info', value: 'visa' },
          ...
          {activeTab === 'visa' && (
            <div className="space-y-6">
              <div className="rounded-2xl border border-secondary/15 bg-slate-900/30 p-5 space-y-3">
                <h4 className="text-xs font-bold text-secondary uppercase tracking-wider">Visa Guidance & Requirement</h4>
                <div className="flex items-center space-x-3 text-xs text-slate-300">
                  <ShieldCheck className="h-5 w-5 text-secondary shrink-0" />
                  <div>
                    <p>Status: <strong>{pkg.visaRequirement}</strong></p>
                    <p className="text-3xs text-slate-400 mt-1">Our agency handles fast-track documentation, interview setup, and forms filing for this destination.</p>
                  </div>
                </div>
              </div>

              <div className="rounded-2xl border border-secondary/15 bg-slate-900/30 p-5 space-y-3">
                <h4 className="text-xs font-bold text-secondary uppercase tracking-wider">Weather Conditions</h4>
                <div className="flex items-center space-x-3 text-xs text-slate-300">
                  <Thermometer className="h-5 w-5 text-secondary shrink-0" />
                  <div>
                    <p>Average climate: <strong>{pkg.weather || 'Pleasant & Sunny'}</strong></p>
                    <p className="text-3xs text-slate-400 mt-1">We recommend packing light cotton wear, high-protection sunscreen, and comfortable trekking footwear.</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <form onSubmit={handleAddReview} className="glass rounded-xl p-4 border border-secondary/20 space-y-3">
                ...
                <div className="flex space-x-2">
                  <input
                    ...
                    placeholder="E.g., Dubai was wonderful, fully escorted with luxury transfers..."
